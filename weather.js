...
let handleWeatherResponse = function(response) {
  ...
  // Clears out the results of the prior search when you do a new search
  $("#current-conditions-icon").empty();
  $("#current-conditions-text").empty();
  $(".forecast").empty();

  // Adds to the contents of the element
  $("#current-conditions-icon").append(icon(response.currently.icon));
  $("#current-conditions-text").append(response.currently.summary);
...
for (let i=0; i<6; i++ )
{
  let forecast = response.daily.data[i]
    ...
    html = html + '<h4>H:'+ Math.round(forecast.apparentTemperatureHigh) + ' | ' + Math.round(forecast.apparentTemperatureLow) + '</h4>'
